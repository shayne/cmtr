...
if [ -d "$CMTR_REPO" ] && [ -f "$CMTR_REPO/pyproject.toml" ]; then
  if ! command -v uv >/dev/null 2>&1; then
    ...
    if ! mise exec -C "$CMTR_REPO" -- uv run --project "$CMTR_REPO" --directory "$CMTR_TARGET" cmtr prepare-commit-msg "$@"; then
      echo "cmtr: command failed; skipping commit message generation" >&2
    fi
  else
    if ! uv run --project "$CMTR_REPO" --directory "$CMTR_TARGET" cmtr prepare-commit-msg "$@"; then
      echo "cmtr: command failed; skipping commit message generation" >&2
    fi
  fi
else
  if command -v uvx >/dev/null 2>&1; then
    if ! uvx cmtr@latest prepare-commit-msg "$@"; then
      echo "cmtr: command failed; skipping commit message generation" >&2
    fi
  else
